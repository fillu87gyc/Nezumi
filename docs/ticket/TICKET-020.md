...
| reason | HTTP | message |
|---|---|---|
| `daily_burst_limit` | 429 | 「本日のOCR残量がなくなりました（1日30枚補充・最大60枚まで繰越）。リセット: {時刻}」 |
| `monthly_limit` | 429 | 「月次OCR上限（900枚）に達しました。リセット: {日付}」 |

> メッセージに「上限60枚」とだけ書くと、繰越なしの日（残30）で実際と食い違うため、補充/繰越の仕組みがわかる文言にする。
...
- バケットは `ocr:bucket` の単一キーで**グローバル**に管理する（Cloudflare Access 配下の単独利用前提のため、ユーザー単位の分離は不要）
- KV は read-modify-write がアトミックではないが、単独利用で同時 OCR リクエストはほぼ発生しないため許容する。
  厳密な原子性が必要になったら Durable Objects に移行する
- `monthlyQuota: 900` は 30 日換算。31 日月では日次補充の合計が 930 になるが、月次キャップ 900 が優先されるため問題ない
